...
class GCurve(pya.PCellDeclarationHelper):
    """
    Coplanar waveguide port (for gerber)
    """

    def __init__(self):
        # Important: initialize the super class
        super(GCurve, self).__init__()
...
def create_curve(obj, start, rotation, radius, angle, right_curve, width, gap, ground, hole):
    ...
    i = 1
    while i*spot < gap:
        for phi in np.linspace(0 if forth else angle, angle if forth else 0, res):

            ux = np.sin(phi*np.pi/180)
            uy = np.cos(phi*np.pi/180)

            path_u_list.append(pya.DPoint(ux*(p_gu-i*spot), uy*(p_gu-i*spot)-mag))
            path_l_list.append(pya.DPoint(ux*(p_gl-i*spot), uy*(p_gl-i*spot)-mag))
        forth = not forth
        i += 1

    for i in range(res):
        path_u_list.append(path_u_list[-2*(i+1)])
        path_l_list.append(path_l_list[-2*(i+1)])

    path_u_list.append(path_u_list[0])
    path_l_list.append(path_l_list[0])

    # path_u_list.append(pya.DPoint(0, p_gu-spot-mag))
    # path_l_list.append(pya.DPoint(0, p_gl-spot-mag))
