...
class GPort(pya.PCellDeclarationHelper):
    """
...
def create_smooth_port(obj, start, length_taper, length_port, width_port, spacing, width_cpw, gap_cpw, ground, hole):
    ...
    y_down, y_up = 0, 0
    i = 1/2
    while i*spot < length_taper:
        y_down = get_height(1-(i+1)*spot/length_taper)*(width_port-width_cpw)/2 + width_cpw/2
        y_up = get_gap(2*y_down*dbu)/dbu + y_down - spot
        y_down = y_down + spot

        path_u_list.append(pya.DPoint(x_offset+length_taper - i*spot, y_up if down else y_down))
        path_u_list.append(pya.DPoint(x_offset+length_taper - i*spot, y_down if down else y_up))

        path_l_list.append(pya.DPoint(x_offset+length_taper - i*spot, -y_up if down else -y_down))
        path_l_list.append(pya.DPoint(x_offset+length_taper - i*spot, -y_down if down else -y_up))

        i += 1
        down = not down

    i = 0
    while i*spot < length_port:
        path_u_list.append(pya.DPoint(x_offset - i*spot, y_up if down else y_down))
        path_u_list.append(pya.DPoint(x_offset - i*spot, y_down if down else y_up))

        path_l_list.append(pya.DPoint(x_offset - i*spot, -y_up if down else -y_down))
        path_l_list.append(pya.DPoint(x_offset - i*spot, -y_down if down else -y_up))
    ...
    if not down:
        ...
        down = not down  # i.e. True

    i = 0
    while i*spot < gap_max:
        path_u_list.append(pya.DPoint(x_offset-length_port - i*spot, y_up if down else -y_up))
        path_u_list.append(pya.DPoint(x_offset-length_port - i*spot, -y_up if down else y_up))
        i += 1
        down = not down
    ...
    y_down = get_height(1/3)*(width_port-width_cpw)/2 + width_cpw/2
    y_up = get_gap(2*y_down*dbu)/dbu + y_down

    path_u_list.append(pya.DPoint(x_offset+length_taper/3, y_down/2 + y_up/2))
    path_l_list.append(pya.DPoint(x_offset+length_taper/3, -y_down/2 - y_up/2))

    y_down = get_height(2/3)*(width_port-width_cpw)/2 + width_cpw/2
    y_up = get_gap(2*y_down*dbu)/dbu + y_down

    path_u_list.append(pya.DPoint(x_offset+length_taper*2/3, y_down/2 + y_up/2))
    path_l_list.append(pya.DPoint(x_offset+length_taper*2/3, -y_down/2 - y_up/2))

    path_u_list.append(path_u_list[1])
    path_l_list.append(path_l_list[1])
