...
class GEnd(pya.PCellDeclarationHelper):
    """
...
def create_end(obj, start, rotation, short, width, gap, ground, hole):
    ...
    if not short:
        ...
        i = 0.5
        while i*spot < gap:
            path_list.append(pya.DPoint(i*spot, p_g-spot if down is True else -p_g+spot))
            path_list.append(pya.DPoint(i*spot, -p_g+spot if down is True else p_g-spot))
            down = not down
            i += 1
    ...
    if not short:
        ...
        # obj.cell.shapes(l4).insert(pya.Path(path_list, 0).transformed(shift))
        obj.cell.shapes(l4).insert(pya.Polygon(path_list).transformed(shift))
